...
- [ ] **T2.1 图布局**：以选中任务为焦点，向上渲染前置链、向下渲染后继链（分层 DAG）。
  最长链 26 级，**必须支持折叠/懒展开**，不要一次画完
- [ ] **T2.2 按奖励反查**：输入"甲板"/"螺丝"等道具名 → 列出所有产出该道具的任务 + 各自完整前置路径
- [ ] **T2.3 交互**：点击节点切换焦点；高亮当前进行中任务
- 判据：从 `F46`（最深链）能正确回溯出 26 级前置且无死循环
