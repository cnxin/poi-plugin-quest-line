...
### M4 字体、描述补全、在线更新 ✅ **已完成（2026-08-21）**

**1. 字体整体加大**（用户反馈看不清）
- 正文 12 → 14px，列表行 12 → 13.5px，标题 15 → 17px
- SVG 节点内 9/10 → 11/12.5px，节点尺寸随之加大：
  `NODE_W 116→146`、`NODE_H 26→34`、`FOCUS_W 168→210`、`GAP_Y 46→58`
- 列表行高 26 → 30px
- ⚠ 节点变大后画布宽度上限从 1200 提到 1500，**测试阈值要同步改**，否则假失败

**2. 任务描述补全**（用户举例 B11：我们只显示「派出…」，quest-info-2 显示「…出击一次」）

根因：kcQuests 有 **`memo2`** 字段（覆盖 573/774 = 74%），是**精确达成条件**，
而我原先只取了 `desc`。对比 B11：

| 来源 | 内容 |
|---|---|
| kanxy（日文） | 新編成した「三川艦隊」で出撃せよ！ |
| kcanotify desc | 派出「鸟海」「青叶」「加古」「古鹰」「天龙」及一艘高速舰的新编成「三川舰队」 |
| **kcQuests memo2** | **使用「鸟海」、「青叶」…以及任一高速舰 出击一次** ← quest-info-2 显示的就是这个 |

处理：新增 `memo` 字段（显示为「达成条件 / 备注」蓝色引用块）；
两源 `desc` 不同时把较详细的存为 `descAlt` 作为补充展示。

**3. 数据源在线更新**（`lib/data-update.es` + 设置页）
- 设置页有「立即更新数据」按钮与「自动更新（每天检查一次）」开关；
  自动更新在 `pluginDidLoad` 延后 5 秒触发，避免拖慢启用
- **只覆盖文本层**（任务名/说明/达成条件/奖励文本）。
  结构化数据（前置关系、奖励对象、编成要求）来自 kanxy 冻结资产，
  远端源没有等价物，覆盖会破坏一致性
- 更新写入 **poi APPDATA 目录**（`APPDATA_PATH/quest-line-data.json`），
  与插件包分离，插件升级不丢更新
- `quest-db.es` 加 `invalidateDb()`，更新后重新加载；
  ⚠ `require` 会缓存并共享对象，**覆盖前必须浅拷贝**，否则污染模块缓存
- 实测：拉取 651 条、应用 107 条差异，前置边与奖励结构完好

**踩坑（目视才发现）：** 在线更新绕过了构建期的 `cleanName`，
更新后任务名的「(月任)」「(日任)」前缀**重新冒出来**。
